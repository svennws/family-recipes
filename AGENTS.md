...
Duplicate an existing JSON file if you need a template. Ensure `publishedAt` stays ISO timestamp-like, `tags` reference existing category folders, and ingredient quantities include units (e.g., `2 dl`, `1 ts`). Embed allergen or dietary notes in `description` rather than the title so filters remain clean.

## Agent Workflow Notes
- Treat `dist/`, `.astro/`, and `node_modules/` as generated output; do not edit them directly.
- CI runs Node.js 24, `npx astro check`, and `npm run build`; use the same checks before considering code or content changes complete.
- Preserve Norwegian recipe titles, slugs, ingredients, and units unless the task explicitly asks for translation or normalization.
- Recipe content must satisfy `src/content.config.ts`; required fields are `title`, `description`, non-empty `ingredients`, and non-empty `steps`.
- Optional recipe fields include `prepTime`, `cookTime`, positive integer `servings`, `servingSize`, `tags`, date-like timestamp fields, and `author`.
- When adding recipes, prefer copying a nearby recipe in the same category so shape, tone, and units stay consistent.
